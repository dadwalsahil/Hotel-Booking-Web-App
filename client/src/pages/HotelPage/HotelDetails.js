...
export const ManagerView = (props) => {
  ...
  const ratings = !hotel.ratings ? 0.0 : hotel.ratings;
  ...
  return (
    <div>
      <Flex>
        ...
        <div className="details" style={{ width: "40%", marginLeft: "20px" }}>
          <Text className="small">
            Location: <span>{hotel.location}</span>
          </Text>
          <Text className="small">
            Price: <span>{getPriceRange()}</span>
          </Text>
        </div>
      </Flex>
      ...
      <div style={{ marginTop: "20px" }}>
        {rooms.map((r) => (
          <RoomDetails room={r} roomNumbers={[]} params={params} />
        ))}
      </div>
    </div>
  );
